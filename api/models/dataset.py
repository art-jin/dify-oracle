...
class Dataset(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(StringUUID, server_default=db.text("SYS_GUID()"))    ##Oracle:StringUUID,在type.py中有定义
    tenant_id = db.Column(StringUUID, nullable=False)
    name = db.Column(db.String(255), nullable=False)
    description = db.Column(db.Text, nullable=True)
    #provider = db.Column(db.String(255), nullable=False, default="vendor")
    provider = db.Column(db.String(255), nullable=False, server_default=db.text("'vendor'"))  ##Oracle
    #permission = db.Column(db.String(255), nullable=False, default="only_me")
    permission = db.Column(db.String(255), nullable=False, server_default=db.text("'only_me'"))  ##Oracle
    ...
    def word_count(self):
        ...
        #    .scalar()
        #)
        #from models.dataset import Document  # 避免循环导入
        try:
            result = db.session.query(
                func.coalesce(
                    func.sum(
                        func.nvl(Document.word_count, 0)  # 使用 Oracle 的 NVL 函数
                    ),
                    literal(0)
                )
            ...
            print(f"Error calculating word_count: {e}")
            return 0
...
class DatasetProcessRule(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(StringUUID, server_default=db.text("SYS_GUID()"))    ##Oracle:SYS_GUID()
    dataset_id = db.Column(StringUUID, nullable=False)
    #mode = db.Column(db.String(255), nullable=False, default="automatic")
    mode = db.Column(db.String(255), nullable=False, server_default=db.text("'automatic'"))  ##Oracle
...
class Document(db.Model):  # type: ignore[name-defined]
    ...
    # pause
    #is_paused = db.Column(db.Boolean, nullable=True, server_default=db.text("false"))#Oracle
    is_paused = db.Column(db.Integer, nullable=True, server_default=db.text("0"))
    ...
    indexing_status = db.Column(db.String(255), nullable=False, server_default=db.text("'waiting'"))  ##Oracle
    #enabled = db.Column(db.Boolean, nullable=False, server_default=db.text("true"))#Oracle
    enabled = db.Column(db.Integer, nullable=False, server_default=db.text("1"))
    disabled_at = db.Column(db.DateTime, nullable=True)
    disabled_by = db.Column(StringUUID, nullable=True)
    #archived = db.Column(db.Boolean, nullable=False, server_default=db.text("false"))#Oracle
    archived = db.Column(db.Integer, nullable=False, server_default=db.text("0"))
    archived_reason = db.Column(db.String(255), nullable=True)
    archived_by = db.Column(StringUUID, nullable=True)
    archived_at = db.Column(db.DateTime, nullable=True)
    updated_at = db.Column(db.DateTime, nullable=False, server_default=func.current_timestamp())
    doc_type = db.Column(db.String(40), nullable=True)
    #doc_metadata = db.Column(db.JSON, nullable=True)   ##Oracle
    doc_metadata = db.Column(AdjustedJSON, nullable=True)
    #doc_form = db.Column(db.String(255), nullable=False, default="")
    doc_form = db.Column(db.String(255), nullable=False, server_default=db.text("'text_model'"))  ##Oracle
    ...
    @property
    def hit_count(self):
        #from models.dataset import DocumentSegment  # 避免循环导入
        ##这个修改使用了 SQLAlchemy 的 case 表达式来处理 sum 可能为 NULL 的情况，这应该能够在 Oracle 数据库中正确工作。
        """
        return (
        ...
        result = db.session.query(func.coalesce(func.sum(DocumentSegment.hit_count), 0)) \
            .filter(DocumentSegment.document_id == self.id) \
            .scalar()
        return result or 0
...
class DocumentSegment(db.Model):  # type: ignore[name-defined]
    ...
    # indexing fields
    #keywords = db.Column(db.JSON, nullable=True)#Oracle
    keywords = db.Column(AdjustedJSON, nullable=True)
    ...
    status = db.Column(db.String(255), nullable=False, server_default=db.text("'waiting'::character varying"))  ##Oracle
    created_by = db.Column(StringUUID, nullable=False)
    created_at = db.Column(db.DateTime, nullable=False, server_default=func.current_timestamp())
    updated_by = db.Column(StringUUID, nullable=True)
    updated_at = db.Column(db.DateTime, nullable=False, server_default=func.current_timestamp())
...
class ChildChunk(db.Model):  # type: ignore[name-defined]
    ...
    # initial fields
    id = db.Column(StringUUID, nullable=False, server_default=db.text("SYS_GUID()")) ##Oracle
    tenant_id = db.Column(StringUUID, nullable=False)
    dataset_id = db.Column(StringUUID, nullable=False)
    document_id = db.Column(StringUUID, nullable=False)
...
class AppDatasetJoin(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(StringUUID, primary_key=True,  nullable=False, server_default=db.text("SYS_GUID()"))    ##Oracle:StringUUID,在type.py中有定义
...
class DatasetKeywordTable(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(StringUUID, primary_key=True,  server_default=db.text("SYS_GUID()"))    ##Oracle:StringUUID,在type.py中有定义
    ...
    data_source_type = db.Column(db.String(255), nullable=False, server_default=db.text("'database'"))  ##Oracle
...
class Embedding(db.Model):  # type: ignore[name-defined]
    ...
    model_name = db.Column(db.String(255), nullable=False, server_default=db.text("'text-embedding-ada-002'"))  ##Oracle
    hash = db.Column(db.String(64), nullable=False)
    embedding = db.Column(db.LargeBinary, nullable=False)
    created_at = db.Column(db.DateTime, nullable=False, server_default=func.current_timestamp())
    #provider_name = db.Column(db.String(255), nullable=False, default="")
    provider_name = db.Column(db.String(255), nullable=False, server_default=db.text("''"))  ##Oracle
...
class DatasetCollectionBinding(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(StringUUID, primary_key=True,
                   server_default=db.text("SYS_GUID()"))  ##Oracle:SYS_GUID()
    provider_name = db.Column(db.String(40), nullable=False)
    model_name = db.Column(db.String(255), nullable=False)
    #type = db.Column(db.String(40), default="dataset", nullable=False)
    type = db.Column(db.String(40), nullable=False, server_default=db.text("'dataset'"))  ##Oracle
...
class TidbAuthBinding(db.Model):  # type: ignore[name-defined]
    ...
    #id = db.Column(StringUUID, primary_key=True, default=lambda: uuid.uuid4())
    id = db.Column(StringUUID, primary_key=True, server_default=db.text("SYS_GUID()"))  ##Oracle:SYS_GUID()
    tenant_id = db.Column(StringUUID, nullable=True)
    cluster_id = db.Column(db.String(255), nullable=False)
    cluster_name = db.Column(db.String(255), nullable=False)
    #active = db.Column(db.Boolean, nullable=False, server_default=db.text("false"))#Oracle
    active = db.Column(db.Integer, nullable=False, server_default=db.text("0"))
    status = db.Column(db.String(255), nullable=False, server_default=db.text("'CREATING'")) ##没有改
...
class DatasetPermission(db.Model):  # type: ignore[name-defined]
    ...
    id = db.Column(StringUUID, primary_key=True,
                   server_default=db.text("SYS_GUID()"))  ##Oracle:StringUUID,在type.py中有定义
    dataset_id = db.Column(StringUUID, nullable=False)
    account_id = db.Column(StringUUID, nullable=False)
    tenant_id = db.Column(StringUUID, nullable=False)
    #has_permission = db.Column(db.Boolean, nullable=False, server_default=db.text("true"))#Oracle
    has_permission = db.Column(db.Integer, nullable=False, server_default=db.text("1"))
    created_at = db.Column(db.DateTime, nullable=False, server_default=func.current_timestamp())

...
class DatabaseConfig(BaseSettings):
    ...
    def SQLALCHEMY_DATABASE_URI(self) -> str:
        ...
        elif self.SQLALCHEMY_DATABASE_URI_SCHEME == 'oracle+oracledb':
            # Oracle 连接字符串构建逻辑
            return (
                f"oracle+oracledb://"
                f"{quote_plus(self.DB_USERNAME)}:{quote_plus(self.DB_PASSWORD)}@{self.DB_HOST}:{self.DB_PORT}"
                f"/?service_name={self.DB_DATABASE}"
            )
        else:
            raise ValueError(f"Unsupported database scheme: {self.SQLALCHEMY_DATABASE_URI_SCHEME}")
...
import json
@computed_field ##Oracle
@property
def SQLALCHEMY_ENGINE_OPTIONS(self) -> dict[str, Any]:
    if self.SQLALCHEMY_DATABASE_URI_SCHEME == "oracle+oracledb":
        return {
            ...
            'json_serializer': json.dumps,#Oracle
            'json_deserializer': json.loads #Oracle
        }
    elif self.SQLALCHEMY_DATABASE_URI_SCHEME == "postgresql":
        return {
            "pool_size": self.SQLALCHEMY_POOL_SIZE,
            "max_overflow": self.SQLALCHEMY_MAX_OVERFLOW,
            "pool_recycle": self.SQLALCHEMY_POOL_RECYCLE,

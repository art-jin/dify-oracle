...
class TagService:
    ...
    def get_tags(tag_type: str, current_tenant_id: str, keyword: Optional[str] = None) -> list:
        ...
        """
        # 定义子查询，用于计算每个 tag 的 binding_count
        subq = select(func.count(TagBinding.id)).where(TagBinding.tag_id == Tag.id).label("binding_count")

        # 主查询
        query = db.session.query(
            Tag.id.label("tags_id"),
            Tag.type.label("tags_type"),
            Tag.name.label("tags_name"),
            subq
        ).filter(
            Tag.type == tag_type,

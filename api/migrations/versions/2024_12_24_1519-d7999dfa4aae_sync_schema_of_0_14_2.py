...
def upgrade():
    ...
    with op.batch_alter_table("tool_providers", schema=None) as batch_op:
        #batch_op.drop_index("unique_tool_provider_tool_name")  #Oracle
        #To resolve the Oracle error ORA-02429 when running the Alembic migration, you need to modify the migration script to drop the unique constraint instead of the index when using Oracle. Here's how to do it:
        bind = op.get_bind()
        # Assuming the constraint and index have the same name
        constraint_name = 'UNIQUE_TOOL_PROVIDER_TOOL_NAME'  # Replace with actual name
